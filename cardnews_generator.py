...
# 시의성 있는 이벤트/트렌드 주제
_TRENDING_TOPICS = [
    "나폴레옹이 전쟁터에서 꼭 챙긴 약의 정체",
    "넷플릭스 사극에 나온 한약, 실제 효능은?",
    "BTS 멤버가 먹는다는 '녹용', 진짜 효과는",
    "비행기 퍼스트클래스에 비치된 한약이 있다?",
    "할리우드 셀럽들의 동양의학 열풍, 그 중심에 공진단",
    "일본 관광객이 한국에서 꼭 사가는 한약 TOP3",
    "명품 브랜드와 한약의 공통 원료 '사향'",
    "수면 앱 1위 나라 한국, 불면증의 한의학적 해법",
    "ChatGPT에게 물어본 '공진단', AI도 인정한 효능",
    "40대 여성 72%가 겪는 증상, 경옥고로 관리하는 법",
]
...
- "나폴레옹이 전쟁터에서 꼭 챙긴 약" → 우황청심원 → 명나라 사신들이 조선에서 역수입
- "조선 최장수 왕 영조가 매일 먹었던 비밀" → 승정원일기 기록 358회 → 경옥고
- "비행기 퍼스트클래스에 비치된 한약" → 시차적응 + 공진단 → 왕실 보약의 현대적 가치
- "BTS 정국이 말한 '녹용' 그게 뭔데?" → MZ 관심 + 녹용 효능 → 성장인자 IGF-1
