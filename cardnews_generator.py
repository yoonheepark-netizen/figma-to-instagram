...
## 최우선 규칙: 주제 힌트 준수!
- "주제 힌트"가 주어지면, 반드시 해당 주제를 중심으로 아이디어를 만드세요.
- {domain} 관점에서 해당 주제를 풀되, 주제 자체를 벗어나지 마세요.
- 힌트가 없으면 자유 주제.
...
## 최우선 규칙: 주제 힌트 준수!
- "주제 힌트"가 주어지면, **6개 아이디어 모두** 해당 주제를 중심으로 생성하세요.
- 주제를 다양한 각도(건강정보, 연예, 생활, 여성, 직장인)에서 풀되, 주제 자체를 벗어나지 마세요.
- 힌트가 "환절기 일교차 건강관리"이면, 6개 모두 환절기/일교차/계절 변화와 관련된 건강 아이디어여야 합니다.
- 힌트가 없으면 자유 주제.
...
def generate_ideas(
    ...
    if topic_hint:
        parts.append(f"\n★ 주제 힌트 (최우선!): {topic_hint}")
        parts.append("→ 6개 아이디어 모두 위 주제를 중심으로 다양한 각도에서 작성하세요.")
    ...
    for key, num in active_cards:
        ...
        try:
            ...
            if key == "cover":
                ...
                _PRODUCT_KW = ("경옥고", "공진단", "총명공진단", "우황청심원", "녹용한약", "녹용", "콜드퀵", "까스퀵")
