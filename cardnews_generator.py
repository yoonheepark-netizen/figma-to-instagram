...
# 절기·시즌별 주제 풀 (날짜 범위 → 추천 주제들)
_SEASONAL_TOPICS = {
    # 겨울 (12~2월)
    "winter": [
        "겨울 한파와 혈액순환의 관계",
        "조선 왕실의 겨울나기 비법",
        "설날 과식 후 소화 돕는 한방 양생법",
        "겨울잠 자는 동물 vs 불면증 현대인",
        "동지 팥죽의 한의학적 비밀",
    ],
    # 봄 (3~5월)
    "spring": [
        "춘곤증, 알고 보면 간(肝)의 SOS 신호",
        "벚꽃 시즌 알레르기와 한의학",
        "봄나물이 약인 이유 — 동의보감 속 산채",
        "황사·미세먼지 시즌, 조선시대엔 어떻게 막았을까",
        "입춘 양생법: 겨울 독소 해독 프로젝트",
    ],
    # 여름 (6~8월)
    "summer": [
        "삼복더위에 왜 삼계탕? 한의학적 이유",
        "에어컨병의 정체 — 냉방이 기혈을 막는 원리",
        "조선 왕이 여름에 마신 특별한 음료",
        "열대야 불면증, 한의학의 심열(心熱) 이론",
        "여름 보양식의 진짜 효과 — 동의보감 기록",
    ],
    # 가을 (9~11월)
    "autumn": [
        "가을 건조함이 폐를 공격하는 메커니즘",
        "추석 명절 후유증, 한의학적 해결법",
        "단풍이 드는 이유와 우리 몸의 노화",
        "환절기 면역력, 경옥고가 조선 왕실에서 쓰인 이유",
        "가을 우울증(秋鬱)의 한의학적 해석",
    ],
}
...
# 절기별 특화 주제
_SOLAR_TERM_TOPICS = {
    "입춘": "입춘 양생법 — 봄 기운 맞이 해독",
    "우수": "우수(雨水) 절기, 몸 속 수분 밸런스",
    "경칩": "경칩에 깨어나는 건 벌레뿐? 우리 몸도 깨워야 할 때",
    "춘분": "낮밤 같아지는 춘분, 음양 밸런스와 건강",
    "청명": "청명 절기, 한의학에서 말하는 '맑은 기운'이란",
    "곡우": "곡우(穀雨) — 봄비가 약이 되는 이유",
    "입하": "입하, 여름 시작 전 체력 비축 전략",
    "소만": "소만 절기 — 한의사가 추천하는 초여름 양생",
    "망종": "망종(芒種) — 씨 뿌리듯 건강도 미리 준비",
    "하지": "하지, 낮이 가장 긴 날의 건강 관리법",
    "소서": "소서 더위 시작, 조선 궁중의 여름 건강법",
    "대서": "대서 폭염, 한의학의 '이열치열' 진짜 원리",
    "입추": "입추, 가을 대비 면역력 충전 타이밍",
    "처서": "처서 — 더위가 가고 건조함이 오는 시기의 양생",
    "백로": "백로 이슬 내리는 절기, 폐 건강 체크리스트",
    "추분": "추분, 음양 전환기의 몸 관리법",
    "한로": "한로 찬 이슬 절기, 관절 건강 주의보",
    "상강": "상강 서리 내리는 절기, 혈액순환 점검",
    "입동": "입동, 겨울 준비 — 한의학 보온 양생법",
    "소설": "소설 첫눈 절기, 신장(腎) 보양의 시작",
    "대설": "대설 — 큰 눈 내리는 시기, 뼈·관절 건강",
    "동지": "동지 팥죽의 한의학적 비밀",
    "소한": "소한 — 1년 중 가장 추운 절기의 양생법",
    "대한": "대한 강추위, 한의학 혈액순환 관리",
}

# 시의성 있는 이벤트/트렌드 주제 (출처 검증 가능한 것만 수록)
_TRENDING_TOPICS = [
    "동의보감에 358회 기록된 경옥고, 조선 왕실의 보약",  # 출처: 승정원일기
    "샤넬 No.5와 공진단의 공통 원료 '사향'의 비밀",  # 출처: 사향(Musk) 성분 사실
    "동의보감이 유네스코 세계기록유산이 된 이유",  # 출처: 유네스코 2009년 등재
    "허준이 쓴 처방전, 400년 뒤 현대 과학이 검증하다",  # 출처: 동의보감 연구 논문
    "일본 '감포'의 원조가 한국 한약이라는 사실",  # 출처: 일본 한방의학 역사
    "영조 83세 장수 비결, 승정원일기가 밝힌 건강 습관",  # 출처: 승정원일기 기록
    "세계보건기구(WHO)가 인정한 전통의학 분류 체계",  # 출처: WHO ICD-11 (2019)
    "조선시대 과거시험 수험생의 컨디션 관리법",  # 출처: 조선 문헌 기록
    "커피 대신 한방차, 카페인 없이 집중력 높이는 원리",  # 일반 건강 상식
    "현대인의 냉방병, 동의보감에 이미 답이 있었다",  # 출처: 동의보감 상한론
]


def suggest_topics() -> list[dict]:
    """현재 시즌/절기/트렌드 기반 주제 추천 목록 반환

    Returns: [{"label": "표시 텍스트", "topic": "주제 힌트 값", "tag": "시즌/절기/트렌드"}, ...]
    """
    ...
    if solar and solar in _SOLAR_TERM_TOPICS:
        suggestions.append({
            "label": f"📅 {_SOLAR_TERM_TOPICS[solar]}",
...
## 임무
{domain} 분야에서 한의원 브랜드 '수(thesoo)'의 Instagram 카드뉴스 아이디어 2개를 제안하세요.

## 최우선 원칙: "의외성 → 한의학 연결"
가장 중요한 것은 **읽는 사람이 "오, 이거 몰랐네!" 하고 저장/공유하는 콘텐츠**입니다.
아래 실제 성공 사례처럼, 의외의 소재를 한의학과 연결하는 방식이 최고 점수를 받아요.

### 실제 성공 사례 (이 수준의 창의성을 발휘할 것!)
- "샤넬 No.5와 공진단의 향기로운 공통점" → 둘 다 사향(Musk) 사용 → 사향의 한의학적 효능
- "조선 최장수 왕 영조가 매일 먹었던 비밀" → 승정원일기 기록 358회 → 경옥고
- "동의보감이 유네스코 세계기록유산이 된 진짜 이유" → 2009 등재 → 400년 전 처방의 현대적 검증
- "허준이 광해군에게 올린 처방전의 비밀" → 실록 기록 → 공진단 원방
- "WHO가 한의학을 국제질병분류에 넣은 이유" → ICD-11(2019) → 전통의학 과학적 인정

### 금칙: 이런 건 재미없어요 (절대 피할 것)
- "면역력 높이는 3가지 방법" (뻔한 건강 정보)
- "공진단의 효능과 성분" (제품 소개 느낌)
- "환절기 건강 관리법" (너무 일반적)
- "스트레스 해소에 좋은 한약" (뻔한 주제)
...
금지: ~입니다, ~습니다, ~이다, ~했다 (원전 인용 「」 내부만 예외)

## 식약처 규제
금지: 치료, 완치, 특효약, 만병통치, 약효, 처방전, 진단

## 신뢰도 원칙 (의료 콘텐츠이므로 필수!)
- 모든 사실/수치/역사적 주장에는 반드시 **검증 가능한 출처**를 source 필드에 기입
- 출처 예시: 동의보감 ○○편, 승정원일기 ○년○월, ○○대학 ○○연구(20XX), WHO ICD-11 등
- 검증 불가능한 주장(유명인 발언, 미확인 통계, 루머)은 절대 사용 금지
- "~라고 알려져 있다" 식의 모호한 출처 금지 — 구체적 문헌/기관명 명시

## 출력 형식 (JSON 배열 2개, 다른 텍스트 없이 JSON만)
```json
[
  {{
    "title": "아이디어 제목 (의외의 소재 → 한의학 연결 형태로)",
    "source": "참고 트렌드/출처",
    ...
    "product": "공진단/경옥고/녹용한약/우황청심원 중 하나",
    "pattern": "패턴명",
...
### 1. 후킹력 (20점) — 가장 중요!
- 20점: 헤드라인만 보고 "이게 뭐야?" 하고 클릭함 (예: "샤넬 No.5와 공진단의 공통점")
- 15점: 관심을 끌지만 의외성이 부족
- 10점: 평범한 건강 정보 헤드라인
- 5점: "~의 효능" 같은 뻔한 제목

### 2. 스토리텔링 (20점)
- 20점: 카드1→7까지 "다음 카드가 궁금한" 서사 구조
- 15점: 흐름이 있지만 긴장감 부족
- 10점: 나열식 정보 전달
- 5점: 연결 없이 각 카드가 따로 노는 느낌

### 3. 타겟공감도 (20점)
- 20점: 30~40대 여성이 "나한테 필요한 정보!" 공감
- 15점: 공감하지만 긴급성 부족
- 10점: 일반적 건강 정보 수준
- 5점: 타겟과 무관한 주제

### 4. 브랜드연결 (20점)
- 20점: 이야기 흐름 속에 제품이 자연스럽게 등장 (비유형 마무리)
- 15점: 연결은 되지만 약간 억지스러움
- 10점: 제품과의 연결이 약함
- 5점: 광고처럼 읽히거나 연결이 없음

### 5. 바이럴가능성 (20점)
- 20점: "이거 친구한테 보내야지!" 수준 — 저장/공유 욕구 유발
- 15점: 유익하지만 공유까지는 아님
- 10점: 일반적 콘텐츠
- 5점: 공유하고 싶지 않은 수준

## 가산점 (최대 +15점, 총점이 100점을 넘을 수 있음)
- 의외의 소재 → 한의학 연결: **+10점** (가장 큰 가산점!)
- "몰랐던 사실" WOW 팩터: +5점
...
```json
[
  {{"index": 0, "hook": 18, "story": 16, "empathy": 17, "brand": 15, "viral": 19, "bonus": 10, "penalty": 0, "total": 95, "comment": "한줄 코멘트"}},
  ...
]
```"""
...
## 신뢰도 원칙 (의료 콘텐츠이므로 필수!)
- sources 필드에 본문에서 인용한 모든 사실/수치/역사의 **구체적 출처**를 기입
- 출처 예시: "동의보감 탕액편", "승정원일기 영조 20년 3월 기록", "한국한의학연구원 2023 연구"
- 검증 불가능한 통계("~% 가 겪는"), 유명인 발언, 루머는 절대 사용 금지
- 각 카드에 담긴 핵심 사실은 반드시 문헌·연구로 뒷받침되어야 함
